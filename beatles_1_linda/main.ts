...
let app:App = async (path, canvas) => {
    ...
    let paulhair = g.texture(await image('paulhair.png'))
    let paulface = g.texture(await image('paulface.png'))
    let linda = g.texture(await image('linda.png'))
    let lindahair = g.texture(await image('lindahair.png'))
    let lindaface = g.texture(await image('lindaface.png'))
        ...
        buf.draw({
            ...
            shader: `
                ...
                vec4 pixel(px p) {
                    ...
                    return hue(blend(bgc, zoomblur(p.bg, p.uv, t, t, t * 0.004)), 0.3);
                }
            `,
        })
        buf.draw({
        ...
        buf.draw({
            ...
            shader: `
                ...
                vec4 pixel(px p) {
                    ...
                    return hue(difference(a, b), 0.60 + huet);
                }
            `
        })
        prevs[0].draw({
