...
let proj = `
    ...
    mat4 proj(float aspect, float time) {
        return perspective(42., aspect, 10., 10000.) * view(time);
...
let app:App = async (path, canvas) => {
    let mode = 0
    // events.click(() => mode = mode? 0 : 1)
    let g = gfx(canvas(1920 * 0.5, 1080 * 0.5))
    let gbuf = g.buffer()
    let circle = g.texture(await image('circle.png'))
    let baseimg = g.texture(await image('base.png'))
    let fontimg = g.texture(await image('font.png'))
    let loveimg = g.texture(await image('love.png'))
    let brushdir = g.texture(await image('brushdir.png'))
    let brush = g.texture(await image('brush.png'))
    let buf = g.buffer()
    ...
    return async (input) => {
        ...
        gbuf.draw(circles(circle, input.time, 700, -400, -1, 1.3, mode))
        gbuf.draw(love(baseimg, fontimg, loveimg, input.time))
        gbuf.draw(circles(circle, input.time, 100, 400, 1.8,  1.5, mode))
        buf.clear()
        buf.draw(rainbow(brush, brushdir, input.time, 1.32, 400, -100, 0.6, 0.6))
        buf.draw(rainbow(brush, brushdir, input.time, 0.42, 600, -200, 0.4, 0.4))
        buf.draw(rainbow(brush, brushdir, input.time, 2.26, 500, -500, 2.4, 1))
        buf.draw(rainbow(brush, brushdir, input.time, 0.66, -840, 400, -1.6, -1))
        gbuf.draw({
            uniforms: [
                ['image', buf.color(0)]
            ],
...
let circles = (sprite:Texture, time:number, instances:number, z:number, zsign:number, scale:number, mode:number):DrawCall => ({
    uniforms: [
        ['time', time],
        ['sprite', sprite],
        ['mode', mode]
...
let love = (base:Texture, font:Texture, image:Texture, time:number):DrawCall => ({
    instances: 20,
    uniforms: [
        ['time', time],
        ['base', base],
    ...
    shader: `
        ...
        vec4 pixel(px p) {
            ...
            a = blend(a, hue(texture(p.font, p.uv), p.time * 4.));
            a = blend(a, texture(p.image, p.uv));
...
let rainbow = (brush:Texture, brushdir:Texture, time:number, offset:number, x:number, y:number, scalex:number, scaley:number):DrawCall => ({
    instances: 2,
    uniforms: [
        ['time', time],
        ['brushdir', brushdir],
