...
let proj = `
    ...
    mat4 proj(float aspect, float time) {
        return perspective(42., aspect, 0.1, 100000.) * view(time);
...
class Imgs {
    constructor(
        ...
        public bg:Texture,
        public bw:Texture,
        public black:Texture,
        public displace:Texture,
        public mask:Texture
    ) {}
}

let layer = (g:Buffer, imgs:Imgs, time:number, front:boolean) => {
    g.draw({
        clear: !front,
        uniforms: [
            ['time', time * 2],
...
let app:App = async (path, canvas) => {
    ...
    let gbuf = g.buffer()
    let imgs = {
        test: g.texture(await image('test.png')),
        bg: g.texture(await image('bg.png')),
        bw: g.texture(await image('bw.png')),
        black: g.texture(await image('black.png')),
        displace: g.texture(await image('displace.png')),
        mask: g.texture(await image('mask.png'))
    }
    let sign = g.texture(await image('droidez.png'))
    return async (input) => {
        layer(gbuf, imgs, input.time, false)
        layer(gbuf, imgs, input.time, true)
